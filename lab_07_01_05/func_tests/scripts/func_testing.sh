...
files="func_tests/data/pos_??_args.txt"
for file_args in $files; do
	number=$(echo "$file_args" | grep -o "[0-9]*")
    ...
    if bash func_tests/scripts/pos_case.sh "$file_out" "$file_args" ;then
...
for file_in in $files; do
    ...
    if bash func_tests/scripts/neg_case.sh "$file_in" "$file_args"; then
